...
function SunpelPredictions({ 
    ...
    if (renderPredictions.length === 0) {
        return (
            <div className="sites-card">
                <DataNotFoundPage props={`No sunpel predictions available for ${formatDisplayDate(todaysDate)}`}/>
                <br/>
                <Adsense
                    client="ca-pub-5665711413000284"
                    slot="3850951453"
...
export async function getServerSideProps() {
    ...
    try {
        ...
        if (data.status === true) {
            ...
            if (data.data && data.data.length > 20) {
                try {
                    ...
                    console.error('Error fetching full batch for sunpel predictions:', batchError);
            ...
            return {
                props: {
                    ...
                    error: data.message || "Failed to load sunpel predictions",
                    baseUrl: baseUrl,
                    todaysDate: todaysDate
                }
            };
        ...
        console.error('Error fetching sunpel predictions:', error);
...
export default SunpelPredictions;