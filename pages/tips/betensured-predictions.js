// pages/betensured-predictions.js
...
import BetensuredPredictionContent from "../../components/seo-content/tips/betensured-predictions";

function BetensuredPredictions({ 
    initialData, 
    endpointStatus, 
    error,
    baseUrl,
    ...
    if (renderPredictions.length === 0) {
        return (
            <div className="sites-card">
                <DataNotFoundPage props={`No betensured predictions available for ${formatDisplayDate(todaysDate)}`}/>
                <br/>
                <Adsense
                    client="ca-pub-5665711413000284"
                    slot="3850951453"
...
export async function getServerSideProps() {
    ...
    try {
        ...
        if (data.status === true) {
            ...
            if (data.data && data.data.length > 20) {
                try {
                    ...
                    console.error('Error fetching full batch for betensured predictions:', batchError);
            ...
            return {
                props: {
                    ...
                    error: data.message || "Failed to load betensured predictions",
                    baseUrl: baseUrl,
                    todaysDate: todaysDate
                }
            };
        ...
        console.error('Error fetching betensured predictions:', error);
...
export default BetensuredPredictions;