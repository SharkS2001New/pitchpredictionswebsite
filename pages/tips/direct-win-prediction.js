// pages/direct-win-predictions.js
...
function DirectWinPredictions({ 
    ...
    if (renderPredictions.length === 0) {
        return (
            <div className="sites-card">
                <DataNotFoundPage props={`No direct win predictions available for ${formatDisplayDate(todaysDate)}`}/>
                <br/>
                <Adsense
                    client="ca-pub-5665711413000284"
                    slot="3850951453"
...
export async function getServerSideProps() {
    ...
    try {
        ...
        if (data.status === true) {
            ...
            if (data.data && data.data.length > 20) {
                try {
                    ...
                    console.error('Error fetching full batch for direct win predictions:', batchError);
            ...
            return {
                props: {
                    ...
                    error: data.message || "Failed to load direct win predictions",
                    baseUrl: baseUrl,
                    todaysDate: todaysDate
                }
            };
        ...
        console.error('Error fetching direct win predictions:', error);
