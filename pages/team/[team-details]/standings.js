        ...
        if(router.isReady){
            ...
            getTeamsDetailsTop(team_details_top_url).then(data =>{
                ...
                if(data){
                    ...
                    fetchTableStandings(data[0].league_id).then(data => {
                        if(data.status == true){
                            var standings_data = data.data;
                
                            setTableStandings(standings_data[0].standings_data);
            ...
            return (
                <React.Fragment>
                    <React.Fragment>
                        ...
                        <div className="sites-card">
                            {table_standings.length >0 ? 
                                <DisplayIndependentLeagueStandings props={table_standings} home_team_id={teamIdInteger} league_name = {teams_top_data[0].league_name} isMobile = {isMobile}/>
                                :<></>
                            }
                        </div>
                    </React.Fragment>
