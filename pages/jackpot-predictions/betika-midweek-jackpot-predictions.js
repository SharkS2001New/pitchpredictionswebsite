...
export async function getServerSideProps() {
    ...
    const cacheFilename = `betika-midweek-jackpot-fixtures.json`;
    ...
    try {
        ...
        console.error("Error fetching Betika midweek jackpot data:", err);
        ...
        if (fs.existsSync(cachePath)) {
            try {
                ...
                console.error('Fallback error for Betika midweek jackpot:', fallbackErr);
            }
        }
    }
...
async function cleanupOldCacheFiles(cacheDir) {
    try {
        ...
        for (const file of files) {
            if (file === 'betika-midweek-jackpot-fixtures.json') {
        ...
        console.error('Error cleaning up Betika midweek cache:', error);
    }
}
