...
import dedupeFixturesById from "../../components/functions/dedupe_fixtures_by_id";
...
function TopFootballFixturesToday({ 
    ...
    const [allData, setAllData] = useState(() => dedupeFixturesById(initialData || []));
    ...
    const loadMoreData = async () => {
        ...
        try {
            ...
            if (chunkData.status === true && chunkData.data && chunkData.data.length > 0) {
                setAllData(prevData => dedupeFixturesById([...prevData, ...chunkData.data]));
    ...
    if (renderPredictions.length === 0 && !loadingMore && !initialData) {
        return (
            <div className="sites-card">
                <DataNotFoundPage props={`No top football predictions available for ${formatDisplayDate(todaysDate)}`}/>
                <br/>
                <Adsense
                    client="ca-pub-5665711413000284"
                    slot="3850951453"
            ...
            <div className="container-fluid">
                <div className="row" style={{backgroundColor: "#edf3f5"}}>
                    ...
                    <div className="col-md-10 col-12">
                        <FilterTodaysTopOverallDoubleChanceUnderOverHTFTPred1x2 url_filter={router.pathname.substring(1)} />
                    </div>
...
export async function getServerSideProps() {
    ...
    return {
        props: {
            initialData: dedupeFixturesById(initialData),
            endpointStatus,
            error,
            baseUrl: baseUrl,
...
export default TopFootballFixturesToday;