...
function SuperTips() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchGames(formatDate(yesterday), 18).then(response => setYesterdaysMatches(response.data)),
      fetchGames(formatDate(today), 18).then(response => setTodaysMatches(response.data)),
      fetchGames(formatDate(tomorrow), 18).then(response => setTomorrowsMatches(response.data))
    ])
          ...
          <React.Fragment>
            <nav aria-label="breadcrumb" className="mb-5 border-bottom">
              ...
              <div className="col-md-12 align-self-center p-static order-2 text-center">
                <h3 className="font-weight-bold text-dark">Super Tips</h3>
              </div>
            </nav>
            <div className="container">
              <div className="overflow-hidden mb-3">
                <h2 className="font-weight-bold text-color-dark line-height-1 mb-0 appear-animation text-center" data-appear-animation="maskUp" data-appear-animation-delay="250">Super Tips Predictions</h2>
              </div>
              {loading ? (
...
export default withAuth(SuperTips);
