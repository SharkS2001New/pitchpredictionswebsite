...
function SuperTips() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchGames(yesterday, 18).then((response) => setYesterdaysMatches(response.data || [])),
      fetchGames(today, 18).then((response) => setTodaysMatches(response.data || [])),
      fetchGames(tomorrow, 18).then((response) => setTomorrowsMatches(response.data || [])),
    ])
