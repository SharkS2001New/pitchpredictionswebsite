...
function Under25() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetchFreePlanGames("fetch_betarazi_under25_fixtures",formatDate(yesterday), 12).then(response => setYesterdaysMatches(response.data)),
      fetchFreePlanGames("fetch_betarazi_under25_fixtures",formatDate(today), 12).then(response => setTodaysMatches(response.data)),
      fetchFreePlanGames("fetch_betarazi_under25_fixtures", formatDate(tomorrow), 12).then(response => setTomorrowsMatches(response.data))
    ])
