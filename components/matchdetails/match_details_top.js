...
function MatchDetailsTop(props){    
    ...
    let livescores_results = DetermineLiveScores(game_details[0],isMobile);
        ...
        <React.Fragment>
            <div className="mb-2"> 
                <div className="col-sm-12 text-left text-nowrap">
                    <div className='container'>
                        <img src={game_details[0].downloaded_country_flag ? game_details[0].downloaded_country_flag : game_details[0].game_details.downloaded_league_logo} 
                        className="img-fluid league-logo" alt={game_details[0].country_name + "-football-predictions"} loading="lazy" />&nbsp;
                            
                        <span style={{fontWeight:"bold",whiteSpace:"break-spaces"}} className="fixturesTextSize">
                            <a href={encodeURI("/country/football-predictions-for-" + game_details[0].country_name.toLowerCase())+"/fixtures"} className="ml-2 aTxt">{game_details[0].country_name.toUpperCase()}</a>
                            &nbsp;:&nbsp;
                            <a href={encodeURI("/league/football-predictions-for-"+game_details[0].country_name.toLowerCase()+"/"+game_details[0].league_name.replace(/\s+/g, '-').toLowerCase()+"-"+game_details[0].league_id+"/fixtures")} className="ml-2 aTxt">{game_details[0].league_name.toUpperCase()}</a>
                        </span>
                    </div>
                </div>
            </div>
            ...
            <div className="row mb-2">
              ...
              <div className="col-6 text-center">
                <span className="text-center matchdetailsTextSize" style={{fontFamily: "Arial", fontWeight: "bold"}}>{DateTimeToUsersTimezone(game_details[0].date)}</span>
              </div>  
              <div className="col-3"></div>          
            </div>
            <div className="row">
                <div className="col-4 text-center">
                    <span className="matchdetailsTextSize mb-2" style={{fontWeight:"bold",whiteSpace:"nowrap"}}>
                        <a href={encodeURI("/team/" + game_details[0].home_team_name.toLowerCase().replace(/\s+/g, '-')+"-"+game_details[0].home_team_id)+"/results"} className="ml-2 aTxt">{game_details[0].home_team_name}</a>
                    </span>
                    <div>
                ...
                <div className="col-4 text-center"> {/**Display all the predictions */}
                    ...
                    <span style={{ fontWeight: "bold" }}>{game_details[0].status_short === "AET" || game_details[0].status_short ? <><br />{extratime_data}</> : game_details[0].status_short === "PEN" ? <><br />{penalty_data}</> : ""}</span>
                    <br/>
                    {livescores_results[1]}
                    <br/>
                    {isMobile ? "" :<br/>}
                    <span className="fixturesTextSize" style={{color:"#B11111", fontWeight:"bold"}}>{game_details[0].status_short === "PEN" || game_details[0].status_short ==="P" ? "AFTER PENALTIES" : game_details[0].status_short ==="AET"? "AFTER EXTRA TIME" : game_details[0].status_short==="NS" ? DateTimeToUsersTimezone(game_details[0].date).split(' ')[1] :  game_details[0].status_long}</span><br/>
                </div>
                <div className="col-4 text-center">
                    <span className="matchdetailsTextSize mb-2" style={{fontWeight:"bold",whiteSpace:"pre-wrap"}}>
                        <a href={encodeURI("/team/" + game_details[0].away_team_name.toLowerCase().replace(/\s+/g, '-')+"-"+game_details[0].away_team_id)+"/results"} className="ml-2 aTxt">{game_details[0].away_team_name}</a>
                    </span>
                    <div>
            ...
            <div className="row">
              <div className="col-12 text-center fixturesTextSize">
                <span style={{fontWeight:"bold",whiteSpace:"break-spaces"}}>Venue: {game_details[0].venue_name}</span><br/>
              </div>
            </div>
            <br/>
        </React.Fragment>
