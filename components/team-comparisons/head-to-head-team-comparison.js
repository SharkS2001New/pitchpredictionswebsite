...
function H2HTeamComparisons(props){
    ...
    useEffect(()=>{ 
        setH2HMatchDetails(props.props);
    ...
    let todaysDate = getFormattedCurrentDate()

    async function fetchH2HLeagues(url2) {
        // Fetch fixtures 
        const response1 = await fetch(url2,{
            method: 'POST',
            body: JSON.stringify({home_team_id: props.home_team_id, away_team_id:  props.away_team_id, fixture_date: todaysDate}),
            headers: headers,
        });
    ...
    const filterByLeagues = async(leagueId) => {
        ...
        try {
            const response1 = await fetch(h2h_url_by_league,{
                method: 'POST',
                body: JSON.stringify({home_team_id: props.home_team_id, away_team_id:  props.away_team_id,league_id:leagueId,fixture_date: todaysDate}),
                headers: headers,
            });
   ...
   const allFixturesFilter = () => {
        //Load original h2h data which is unfiltered
        setH2HMatchDetails(props.props);
   ...
   ComputePercentages();
    ...
    if(h2h_match_details.length >0){
        {h2h_match_details.slice(0, postNum).map((h2h_match_d,q) => ( 
            ...
            <div className="responsive-row fixturesTextSize" key={q} style={{cursor : "auto"}}>
    ...
    return (
        ...
        <React.Fragment>
                ...
                {loading == false ? h2hmatchdetailslist : <InPagePreLoader/>}

                {h2h_match_details.length >10 ?   
...
export default H2HTeamComparisons;