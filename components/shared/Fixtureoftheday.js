...
import {
  setFixtureOfTheDayCache,
  useFixtureOfTheDayCache,
} from "../functions/fixture_of_the_day_cache";
...
function FixtureOfTheDay() {
  ...
  const cached = useFixtureOfTheDayCache(currentDate);
  const [liveState, setLiveState] = useState(null);

  const activeState = liveState ?? cached;
  const gamesfixtures = activeState?.fixture ?? null;
  const endpointStatus = activeState?.status ?? "";
  const isPrimaryResponse = activeState?.isPrimary ?? false;
  const cacheInfo = activeState?.cacheInfo ?? null;
  ...
  const persistFixtureState = (nextState) => {
    setFixtureOfTheDayCache(currentDate, nextState);
  };

  const applyFixturePayload = (data) => {
    ...
    if (Array.isArray(fixtureData) && fixtureData.length > 0) {
      ...
      persistFixtureState(emptyState);
      return;
    }

    const nextState = {
      fixture: fixtureData ?? null,
      status: data.status === true ? "success" : data.status,
  ...
  const loadFixtureData = async () => {
    try {
      ...
      setLiveState(errorState);
      persistFixtureState(errorState);
    }
  };
  ...
  if (endpointStatus === "success") {
    if (!gamesfixtures) {
      return (
        <div
          className="row"
          style={{ backgroundColor: "#202c3c", color: "white", cursor: "auto" }}
        >
          <div
    ...
    const myNewDateString = formatFixtureDateTime(resolveFixtureDateTime(gamesfixtures));
    ...
    return (
      <div className="row" style={{ backgroundColor: "white", cursor: "auto" }}>
        ...
        <div
          className="responsive-row"
          style={{ backgroundColor: "#202c3c", color: "white" }}
        >
          <span
            style={{
              fontSize: "15px",
        ...
        >
          <div
            className="responsive-cell team-link-standings mb-4"
            title={gamesfixtures.country_name || ""}
          >
            <span style={{ fontSize: "12px", cursor: "default" }}>
              {gamesfixtures.league_short_name || ""}
            </span>
            <br />
          ...
          <div
            className="responsive-cell team-link-namel"
            style={{ textAlign: "left", fontSize: "14px", fontWeight: "bold" }}
          >
            <a
              href={
                "/match/football-predictions-" +
                encodeURIComponent(
                  gamesfixtures.home_team_name?.replace(/\s+/g, "-").toLowerCase() +
                    "-vs-" +
                    gamesfixtures.away_team_name?.replace(/\s+/g, "-").toLowerCase() +
                    "-" +
          ...
          <div
            className="responsive-cell team-link-y"
          ...
          >
            ...
            <br />
            <span
              className={gamesfixtures.goals_home != null ? "scores-card" : ""}
              style={{ color: "black", whiteSpace: "nowrap" }}
            >
              {gamesfixtures.goals_home != null
                ? gamesfixtures.goals_home + " - " + gamesfixtures.goals_away
                : " - "}
            </span>
          </div>
        </div>
  ...
  if (endpointStatus === "No game available") {
    return (
      <div
        className="row"
        style={{ backgroundColor: "#202c3c", color: "white", cursor: "auto" }}
      >
        ...
        >
          <div
            ...
            style={{ marginTop: "20px", textAlign: "center", fontSize: "16px" }}
          >
            <span>No game available for today</span>
            <br />
          </div>
        </div>
      </div>
...
export default memo(FixtureOfTheDay);
